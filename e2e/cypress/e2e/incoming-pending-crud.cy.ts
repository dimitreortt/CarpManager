...
describe("Pending Incoming CRUD Operations", () => {
	let incomingData: {
		name: string
		amount: number
		date: string
	...
	it("should create, view, edit and delete a pending incoming", async () => {
		cy.fixture("testUsers").then((users) => {
			cy.login(users.testUser.email, users.testUser.password)
		})

		cy.visit("/finance/incoming/form?status=pending")
		cy.get('input[name="name"]').type(incomingData.name)

		cy.window().then((win) => {
			;(win as any).pendingIncomingFormInstance.setValue("amount", incomingData.amount)
		})
		...
		cy.visit("/finance/incoming?tab=pending")
		...
		cy.url().should("include", "/finance/incoming/view")
		...
		cy.url().should("include", "/finance/incoming/form")
