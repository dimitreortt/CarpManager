...
describe("Pending Outgoing CRUD Operations", () => {
	let outgoingData: {
		name: string
		amount: number
		date: string
	...
	it("should create, view, edit and delete a pending outgoing", async () => {
		cy.fixture("testUsers").then((users) => {
			cy.login(users.testUser.email, users.testUser.password)
		})

		cy.visit("/finance/outgoing/form?status=pending")
		cy.get('input[name="name"]').type(outgoingData.name)

		cy.window().then((win) => {
			;(win as any).pendingOutgoingFormInstance.setValue("amount", outgoingData.amount)
		})
		...
		cy.visit("/finance/outgoing?tab=pending")
